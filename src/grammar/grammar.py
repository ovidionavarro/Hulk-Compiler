...
simple_program %= function + identifier + parameters + function_style, lambda h,s:FunctionNode(s[2],s[3],s[4][1],s[4][0])
simple_program %= type_ + identifier + type_def, lambda h,s:TypeNode(s[2],s[3][3],s[3][0],s[3][1],s[3][2])
...
#
class_declaration %= variable + equal + main_expression, lambda h,s:TypeAttributeNode(s[1],s[3])
class_declaration %= identifier + parameters + function_style, lambda h,s:FunctionNode(s[1],s[2],s[3][1],s[3][0])
#
protocol_declare %= protocol + identifier + lbrace + protocol_body + rbrace, lambda h,s:ProtocolNode(s[2],s[4])
protocol_declare %= protocol + identifier + extends + identifier + lbrace + protocol_body + rbrace, lambda h,s:ProtocolNode(s[2],s[6],s[4])
#
protocol_body %= identifier + typed_parameters + colon + identifier + semicolon, lambda h,s:[ProtocolMethodNode(s[1],s[2],s[4])]
protocol_body %= identifier + typed_parameters + colon + identifier + semicolon + protocol_body, lambda h,s:[ProtocolMethodNode(s[1],s[2],s[4])]+s[6]
#
typed_parameters %= lparen + rparen, lambda h,s:[]
typed_parameters %= lparen + typed_parameter_list + rparen, lambda h,s:s[2]
#
typed_parameter_list %= identifier + colon + identifier, lambda h,s:[ParameterNode(s[1],s[3])]
typed_parameter_list %= identifier + colon + identifier + typed_parameter_list, lambda h,s:[ParameterNode(s[1],s[3])]+s[4]
#
...
arithmetic_expression %= arithmetic_expression + plus + module, lambda h,s:AritmethicExpression('+',s[1],s[3])
arithmetic_expression %= arithmetic_expression + minus + module, lambda h,s:AritmethicExpression('-',s[1],s[3])

module %= product, lambda h,s:s[1]
module %= module + modulus + product, lambda h,s:AritmethicExpression('%',s[1],s[3])
#
product %= monomial, lambda h,s:s[1]
product %= product + star + monomial, lambda h,s:AritmethicExpression('*',s[1],s[3])
product %= product + divide + monomial, lambda h,s:AritmethicExpression('/',s[1],s[3])
product %= product + int_divide + monomial, lambda h,s:AritmethicExpression('//',s[1],s[3])
#
monomial %= pow_, lambda h,s:s[1]
monomial %= minus + monomial, lambda h,s:AritmethicExpression('-',NumberNode(0),s[2])
#
pow_ %= high_hierarchy_object, lambda h,s:s[1]
pow_ %= pow_ + power_asterisk + high_hierarchy_object, lambda h,s:AritmethicExpression('**',s[1],s[3])
pow_ %= pow_ + power + high_hierarchy_object, lambda h,s:AritmethicExpression('^',s[1],s[3])
#
high_hierarchy_object %= object_exp, lambda h,s:s[1]
high_hierarchy_object %= high_hierarchy_object + as_ + identifier, lambda h,s:AsNode(s[1],s[3])
#
...
function_stack %= string, lambda h,s:StringNode(s[1])
...
object_exp %= identifier, lambda h,s:VariableNode(s[1])
